<template>
    <div class="Flow-container">
      <section id="circle-container">
        <div class="btn">
          <p :class="{ active: currentPage === 'IP'||currentPage === 'IPAlert' }" @click="setCurrentPage('IP')">IP</p>
  ...
  let currentPage = ref(offlineStore.IPAlertString);
  let currentComponent = ref(offlineStore.IPAlert);
  ...
  function setCurrentPage(name: string): void {
    ...
    offlineStore.IPAlertString = name;
    currentPage.value = name;
  
    // 检查是否存在对应的组件
    if (componentMap[name]) {
      currentComponent.value = componentMap[name];
      offlineStore.IPAlert = componentMap[name];
    } else {
