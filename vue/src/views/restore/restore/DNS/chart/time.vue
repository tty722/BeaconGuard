...
const processBackendData = (
  ...
  data.forEach((item) => {
    ...
    if (periodIndex !== -1 && item.protocol.toLowerCase() === 'dns') {
      protocolCounts[periodIndex]['DNS']++;
    }
  });
...
onMounted(() => {
  setTimeout(async () => {
    if (chart.value instanceof HTMLDivElement) {
      ...
      await restoreStore.DNSProtocolData();
      let data = restoreStore.DNS_protocol;
