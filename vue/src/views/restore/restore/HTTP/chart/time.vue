...
const processBackendData = (
  ...
  data.forEach((item) => {
    ...
    if (periodIndex !== -1 && item.protocol.toLowerCase() === 'http') {
      protocolCounts[periodIndex]['HTTP']++;
    }
  });
...
onMounted(() => {
  setTimeout(async () => {
    if (chart.value instanceof HTMLDivElement) {
      ...
      await restoreStore.HTTPProtocolData();
      let data = restoreStore.HTTP_protocol;
      console.log(data);
